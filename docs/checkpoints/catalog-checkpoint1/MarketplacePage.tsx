...
export const MarketplacePage: React.FC<MarketplacePageProps> = ({
    ...
    const loadFilterOptions = async () => {
      try {
        ...
        if (marketplaceType === 'courses') {
          const categoryOptions = getDtmaCategories().map((category) => ({
            id: category.slug,
            name: category.name,
          }));
  ...
  useEffect(() => {
    const loadItems = async () => {
      ...
      try {
        ...
        if (marketplaceType === "courses") {
          ...
          const courseList = getDtmaCourses(courseFilters);
    ...
    loadItems();
  }, [productData, filters, searchQuery, marketplaceType, activeFilters, filterConfig]);
