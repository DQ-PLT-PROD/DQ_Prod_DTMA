  ...
  const handleSocialLogin = async (provider: Provider) => {
    ...
    try {
      ...
      if (err.message?.includes('not enabled')) {
        setError(`${provider.charAt(0).toUpperCase() + provider.slice(1)} sign-in is not enabled. Please contact support.`)
      } else if (err.message?.includes('Invalid')) {
        setError(`${provider.charAt(0).toUpperCase() + provider.slice(1)} configuration error. Please contact support.`)
      } else {
