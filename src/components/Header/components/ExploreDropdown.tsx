...
export function ExploreDropdown({ isCompact = false }: ExploreDropdownProps) {
  ...
  useEffect(() => {
    fetchPublishedCoursesForNav().then(setCourses);
  }, []);
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    switch (event.key) {
      ...
      case "ArrowUp":
        event.preventDefault();
        setFocusedIndex((prev) => (prev <= 0 ? courses.length - 1 : prev - 1));
        break;
      case "Enter":
      case " ":
        event.preventDefault();
          ...
          <div className="max-h-[28rem] overflow-y-auto">
            {courses.map((course, index) => (
              <a
                ...
                href={`/courses?course=${encodeURIComponent(course.slug)}`}
                ...
                onClick={(e) => {
                  e.preventDefault();
                  handleInternalNav(`/courses?course=${encodeURIComponent(course.slug)}`);
                }}
                onMouseEnter={() => setFocusedIndex(index)}
                onFocus={() => setFocusedIndex(index)}
              >
                <div className="flex-shrink-0 mt-0.5">
                  <BookOpenIcon size={20} className="text-[color:var(--md-primary)]" />
                </div>
