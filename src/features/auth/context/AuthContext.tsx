...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const extractUserFromAccount = async (
    account: any
  ...
  useEffect(() => {
    if (!useMockAuth && !bypassMode && accounts.length > 0) {
      if (!instance.getActiveAccount()) {
        console.log(
          "🔄 Setting active account from accounts array:",
          accounts[0]
        );
  ...
  useEffect(() => {
    ...
    const checkOnboarding = async () => {
      ...
      if (error) {
        console.warn(
          "Onboarding gate: profile fetch failed, allowing portal access.",
          error
        );
        if (isSigninRoute) {
          navigate("/portal/my-courses/in-progress", { replace: true });
  ...
  useEffect(() => {
    ...
    const callbackId = instance.addEventCallback((event: EventMessage) => {
      ...
      if (event.eventType === EventType.LOGIN_SUCCESS) {
        ...
        if (result?.account) {
          console.log(
            "👤 Setting active account from LOGIN_SUCCESS event:",
            result.account.username
          );
          instance.setActiveAccount(result.account);

          // Immediately extract user info and update state
          extractUserFromAccount(result.account)
            .then((userProfile) => {
              if (userProfile) {
                console.log(
                  "✅ User state updated from LOGIN_SUCCESS event:",
                  userProfile.email
                );
                setCurrentUser(userProfile);
  ...
  const login = async () => {
    console.log("🔐 Login function called with modes:", {
      useMockAuth,
      bypassMode,
    });
  ...
  const logout = () => {
    console.log("🚪 Logout function called with modes:", {
      useMockAuth,
      bypassMode,
    });
