...
const InProgressPage: React.FC = () => {
  ...
  const handleCourseResume = async (courseSlug: string) => {
    ...
    try {
      ...
      if (resumeLessonId) {
        navigate(
          `/portal/learning/${courseSlug}?resumeLessonId=${encodeURIComponent(
            resumeLessonId
          )}`
        );
        return;
      ...
      console.warn(
        "Resume snapshot failed, falling back to first lesson.",
        error
      );
    } finally {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">
            In progress and ready to resume
          </p>
        </div>
        ...
        <div className="mb-8">
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#1839AD]"></div>
            ...
            <div className="space-y-3">
              {enrollments.map((enrollment) => {
                const progressPct =
                  enrollment.actualProgress?.progressPct ??
