...
const CoursePlayerPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadCourseData = async () => {
      try {
        setIsLoading(true);
        const [fetchedCourse, fetchedLessons, fetchedResources] =
          await Promise.all([
        ...
        const resumeIndex = resumeLessonId
          ? fetchedLessons.findIndex(
              (lesson) => String(lesson.id) === resumeLessonId
            )
          : -1;
        ...
        if (databaseUser?.id) {
          try {
            const { lessonProgress } = await getUserCourseProgress(
              databaseUser.id,
              courseId
            );
    ...
    const preloadAllDurations = async () => {
      ...
      const durationPromises = lessons.map(async (lesson, index) => {
        ...
        if (lesson.duration !== "--:--" && lesson.duration !== "") {
          console.log(
            `✅ Lesson ${index}: Already has duration: ${lesson.duration}`
          );
          return { index, duration: lesson.duration };
        }
        try {
          console.log(
            `🎬 Lesson ${index}: Fetching duration for ${lesson.videoUrl}`
          );
  ...
  useEffect(() => {
    const checkEnrollmentStatus = async () => {
      ...
      try {
        ...
        if (!enrolled) {
          console.log(
            "🚫 User not enrolled, redirecting to course details page"
          );
      ...
      if (isEnrolled && databaseUser?.id && courseId) {
        try {
          ...
          const newCompletedCount =
            lessons.filter((l) => l.completed).length + 1;
  ...
  return (
    <div className="flex h-full w-full">
      ...
      <div
        className={`${
          moduleOpen ? "w-80" : isTheater ? "w-0" : "w-12"
        } shrink-0 bg-white border-r border-gray-200 transition-all duration-300 hidden lg:flex flex-col ${
          showQuiz ? "opacity-50 pointer-events-none" : ""
        } rounded-none`}
      >
        {moduleOpen ? (
      ...
      <main
        className={`flex-1 min-w-0 ${
          isTheater ? "p-0 h-full" : "p-3 md:p-4"
        } overflow-y-auto`}
      >
        {isLoading ? (
          <PageLoader message="Loading course..." variant="minimal" />
        ) : !activeLesson ? (
          ...
          >
            ...
            >
              ...
              <VideoPlayer
                ...
                className={
                  isTheater ? "h-full rounded-none border-0 shadow-none" : ""
                }
              />
            </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm space-y-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  ...
                  title={
                    !isNextLessonUnlocked && !atLastLesson
                      ? "Watch until 1 minute before the end to unlock next lesson"
                      : ""
                  }
                >
                  Next Lesson
              ...
              <div>
                <div className="flex items-center justify-between text-sm mb-2">
                  ...
                  <span className="text-[#1839AD] font-bold">
                    {boundedProgress}%
                  </span>
                </div>
