...
import {
  FilterSidebar,
  FilterConfig,
  FilterOption as SidebarFilterOption,
...
export const CourseCatalogPage: React.FC<CourseCatalogPageProps> = ({
        ...
        const toSidebarOptions = (nodes: IndustryNode[]) =>
          nodes.map((node) => ({
            ...
            children: node.children
              ? toSidebarOptions(node.children)
              : undefined,
          }));

        // Use canonical config for filter shape, refresh categories from DB
        const filterOptions: FilterConfig[] = config.filterCategories.map(
          (fc) =>
            fc.id === "category"
              ? {
                  ...fc,
  ...
  // Sync filters to URL when they change (for deep-linking support)
  useEffect(() => {
    if (filterConfig.length === 0) return; // Wait for filter config to load
  ...
  }, [filters, searchQuery, filterConfig.length, navigate, location.pathname]);
    ...
    const loadItems = async () => {
      ...
      try {
        ...
        const cleanFilters = Object.fromEntries(
          Object.entries(serverFilters).filter(
            ([_, v]) =>
              v !== undefined &&
  ...
  const activeChips = useMemo(() => {
    ...
    Object.entries(filters).forEach(([key, val]) => {
      ...
      selectedValues.forEach((selected) => {
        ...
        const optionLabel =
          findOptionName(config?.options, selected) || selected;
