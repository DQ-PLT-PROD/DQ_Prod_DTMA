...
interface CourseOutlineProps {
    ...
    progressPct: number;
    /** 'sidebar' for desktop side panel, 'stacked' for mobile full-width */
...
export const CourseOutline: React.FC<CourseOutlineProps> = ({
    ...
    return (
        <aside className={`bg-white ${isMobile ? 'border-0' : 'border border-gray-200'} rounded-none overflow-hidden flex flex-col ${isMobile ? '' : 'h-full sticky top-4'}`}>
            ...
            <div
                className={`transition-all duration-300 ${moduleOpen || isMobile ? "max-h-[calc(100vh-200px)]" : "max-h-0"
                    ...
                    {lessons.map((lesson, idx) => {
                        ...
                        return (
                            <button
                                ...
                                disabled={!canAccess}
                                className={`w-full text-left rounded-xl px-3 ${isMobile ? 'py-4' : 'py-3'} border transition flex items-start gap-3 relative ${isActive
