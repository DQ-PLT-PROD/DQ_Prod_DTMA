...
export function CoursesSection() {
    ...
    const updateCourseStatus = async (course: CourseRow, status: 'published' | 'draft') => {
        const hasPermission = status === 'published' ? canPublishCourse : canUnpublishCourse;
        if (!hasPermission) {
            setToast({
                type: 'error',
                message: `You do not have permission to ${status === 'published' ? 'publish' : 'unpublish'} courses.`,
            });
            return;
        }
                                    ...
                                    <td className="px-4 py-3 text-right text-sm font-medium">
                                        <div className="flex items-center justify-end gap-1">
                                            {course.status !== 'published' && canPublishCourse && (
                                                <button
                                                    onClick={() => void updateCourseStatus(course, 'published')}
