...
import { fetchCourseWithContent, fetchRelatedCourses, fetchCourseLessons, fetchCategories } from "../features/courses/services/courseService";
import { Course, Category } from "../types/dtma-lms";

export interface UseCourseDetailsArgs {
  itemId?: string;
...
export function useCourseDetails({
  ...
  const mapCourseToItem = (course: Course, courseLessons: any[] = [], courseResources: any[] = []): ProductItem | null => {
    ...
    return {
      ...
      duration: lessonList.length > 0 ? "Calculating..." : formatDuration(course.estimatedDurationMinutes),
      estimatedDurationMinutes: course.estimatedDurationMinutes,
      lessonCount: lessonList.length
        ? lessonList.filter(l => l.type !== 'intro' && l.type !== 'outro').length
        : course.lessonCount,
