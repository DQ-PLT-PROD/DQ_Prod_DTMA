...
import { fetchFullCourse, fetchRelatedCourses } from '../features/courses/services/courseService';
...
export function useCourseDetails({ itemId, shouldTakeAction = true }: UseCourseDetailsParams): UseCourseDetailsReturn {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (courseAny?.relatedCourses && courseAny.relatedCourses.length > 0) {
        const related = await fetchRelatedCourses(courseAny.relatedCourses);
